...
const PING_TARGET_URL = RENDER_EXTERNAL_URL ? `${RENDER_EXTERNAL_URL}/` : null; // Ping the root path
...
const startKeepAlive = () => {
  // Only start in a production-like environment (Render sets RENDER_INSTANCE_ID)
  // and if RENDER_EXTERNAL_URL is available.
  if (process.env.NODE_ENV === "production" && RENDER_EXTERNAL_URL) {
    console.log(
      `[Keep-Alive-Self] Production environment detected. Starting self keep-alive task for URL: ${PING_TARGET_URL} with interval ${PING_INTERVAL_MS / 1000 / 60} minutes.`
    );
    // Perform an initial ping almost immediately, then set interval
